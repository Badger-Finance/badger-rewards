...
def test_check_tx_receipt():
    ...
    for network_info in active_networks:
        logger.info(f"Checking to see if valid tx {network_info['valid_tx']} is on the {network_info['network']} network")
        assert is_transaction_found(web3, network_info["valid_tx"], 2, network_info["network"])
        with pytest.raises(Exception):
            is_transaction_found(web3, network_info["invalid_tx"], 2, network_info["network"])
