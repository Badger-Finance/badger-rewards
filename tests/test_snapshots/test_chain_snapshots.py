...
def responses_mock_token_balance():
    with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
        for network in BOOST_CHAINS:
            rsps.add(
                responses.GET,
                f"{badger_api}/tokens?chain={network}",
                json={"name": "bBadger"},
                status=200,
            )
...
def test_chain_snapshot__happy(
    mock_fetch_ch_balances, chain, responses_mock_token_balance
):
...
@pytest.mark.parametrize("chain", [Network.Ethereum, Network.Arbitrum])
@pytest.mark.parametrize("snapshots_number", [3, 6, 5])
...
@pytest.mark.parametrize("snapshots_number", [14, 20, 100])
def test_total_harvest_sett_snapshot__invalid_rate(
    ...
    expected_amount: Decimal = (
        Decimal(list(BALANCES_DATA[BBADGER_ADDRESS].values())[0]) * 2
    )
    assert list(snapshot.balances.values())[0] == approx(expected_amount)


def test_total_harvest_sett_snapshot__uneven_balance(
    chain, mocker, responses_mock_token_balance
):
